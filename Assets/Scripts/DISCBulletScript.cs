...
public class DISCBulletScript : MonoBehaviour, IBullet
{
    ...
    void OnCollisionEnter2D(Collision2D c){
        if (c.gameObject.tag=="Environment"){
            //collision/bounce with environment code goes here probably
            //change rotation of the bullet when it hits environment(it is going in the direction of transform.up)
            bounces++;
        }
    }
